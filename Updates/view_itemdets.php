...
<body>
     ...
     <header>
        ...
        <h1>Open Account or Login to buy</h1>
        <nav>
            <ul>
...
document.querySelector("button#send-offer").addEventListener("click", () => {
  ...
  if (offerValue >= sellerAcceptanceThreshold) {
    ...
    document.getElementById("accept-counteroffer").addEventListener('click', () => {
      ...
      xhr2.open("POST", "updateoffer.php", true);
...
function showAddToCart(agreedPrice) {
    ...
    const itemId = urlParams.get("id"); // Extracts the `id` parameter

    addToCartButton.onclick = () => {
        const params = new URLSearchParams();
        params.append("item_id", itemId);
        params.append("price", agreedPrice);
        params.append("quantity", 1);

        console.log("Sending data to server:", params.toString());
        ...
        .catch((error) => {
            ...
            messageBox.innerHTML += `<p>An error occurred while adding the item to the cart. Please try again later.</p>`;
        });
...
document.addEventListener("DOMContentLoaded", () => { 
    document.querySelector(".buy-now").addEventListener("click", (e) => {
        ...
        const itemDetails = {
            ...
            item_price: document.querySelector("input[name='item_price']").value, // Make sure the price is captured correctly
        };

        // Send AJAX request
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "add2cart4BN.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    alert(xhr.responseText.trim()); // Show response from the server

                    // Make the hidden Add to Cart button visible
